...
<body>
    ...
    <script>
        ...
        async function carregarRankingGlobal() {
            ...
            } catch (err) { elListaRanking.innerHTML = "Erro ao carregar ranking."; }
        }
        ...
        async function enviarPontuacaoServidor() {
            ...
            try {
                await fetch('/api/salvar', {
                    ...
                    body: JSON.stringify({ nome: nomeJogador, pontos: pontos }) // CORRIGIDO AQUI (pontos em vez de points)
                });
        ...
        function loopJogo(tempoAtual) {
            ...
            for (let i = 0; i < corpo.length; i++) {
                if (corpo[i].x === x && body && corpo[i].y === y) { morreu(); return; }
            }

            // Garante que a colisão com o corpo funcione de forma simples
        ...
        function mudarDirecao(novaDir) {
            if (novaDir === "C" && direcaoAtual !== "BAIXO") { vx = 0; vy = -BLOCO; direcaoAtual = "CIMA"; }
            if (novaDir === "B" && direcaoAtual !== "CIMA") { vx = 0; vy = BLOCO; direcaoAtual = "BAIXO"; }
            if (novaDir === "E" && direcaoAtual !== "DIREITA") { vx = -BLOCO; vy = 0; direcaoAtual = "ESQUERDA"; }
            if (novaDir === "D" && direcaoAtual !== "ESQUERDA") { vx = BLOCO; vy = 0; direcaoAtual = "DIREITA"; }
        }

        window.addEventListener("keydown", e => {
            if (estado !== "JOGANDO") return;
            if (["ArrowUp", "KeyW"].includes(e.code)) mudarDirecao("C");
            if (["ArrowDown", "KeyS"].includes(e.code)) mudarDirecao("B");
            if (["ArrowLeft", "KeyA"].includes(e.code)) mudarDirecao("E");
            if (["ArrowRight", "KeyD"].includes(e.code)) mudarDirecao("D");
        });

        document.querySelectorAll(".btn-dir").forEach(btn => {
            btn.addEventListener("touchstart", (e) => { e.preventDefault(); mudarDirecao(btn.getAttribute("data-dir")); });
            btn.addEventListener("click", () => mudarDirecao(btn.getAttribute("data-dir")));
        });
